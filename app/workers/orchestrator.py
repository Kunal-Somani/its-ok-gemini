...
class TaskOrchestrator:
    ...
    def _run_safety_gate(
        self, existing_code: str, generated_files: Dict[str, str], instruction: str
    ) -> None:
    ...
    async def _notify_evaluator(self, task: TaskRecord) -> None:
        ...
        if not task.evaluation_url:
            logger.warning(
                "evaluator_notification_skipped_no_url", task_id=str(task.id)
            )
            return
        ...
        try:
            ...
            logger.warning(
                ...
                task_id=str(task.id),
                error=str(e),
                attempt=self._notify_evaluator.retry.statistics.get(
                    "attempt_number", 1
                ),
            )
            raise
                ...
                try:
                    ...
                    with temporary_clone_dir() as temp_dir:
                        ...
                        with tracer.start_as_current_span("save_attachments"):
                            if attachments:
                                os.makedirs(
                                    os.path.join(temp_dir, "assets"), exist_ok=True
                                )
                                for idx, att in enumerate(attachments):
                                    ext = att.get("mime_type", "image/png").split("/")[
                                        -1
                                    ]
                                    att_path = os.path.join(
                                        temp_dir, "assets", f"attachment_{idx}.{ext}"
                                    )
                        ...
                        with tracer.start_as_current_span(
                            ...
                            generate_span.set_attribute(
                                "task.round_index", task.round_index
                            )
                            generate_span.add_event(
                                "status_transition", {"status": "GENERATING"}
                            )
                        ...
                        if task.round_index > 1 and existing_code:
                            with tracer.start_as_current_span("safety_gate"):
                                self._run_safety_gate(
                                    existing_code, generated_files, instruction
                                )
                        ...
                        with tracer.start_as_current_span("task.deploy") as deploy_span:
                            ...
                            deploy_span.add_event(
                                "status_transition", {"status": "DEPLOYING"}
                            )
                            git_manager.add_all()
                            git_manager.commit(
                                message=f"feat: Round {task.round_index} automated updates"
                            )
                            git_manager.push()
