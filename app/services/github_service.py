...
class GitManager:
    ...
    def commit(self, message: str = "feat: automated code generation update", author_name: str = "Gemini Worker", author_email: str = "bot@its-ok-gemini.local"):
...
class GitHubService:
    def __init__(self):
        ...
        try:
            ...
            self.private_key = base64.b64decode(settings.GITHUB_PRIVATE_KEY_B64).decode("utf-8")
    ...
    async def get_installation_access_token(self) -> str:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if inst_resp.status_code != 200:
                logger.error("github_installation_fetch_failed", status=inst_resp.status_code, body=inst_resp.text)
                raise GitHubServiceError(f"Failed to find installation for {self.username}")
            ...
            if token_resp.status_code != 201:
                logger.error("github_token_generation_failed", status=token_resp.status_code, body=token_resp.text)
                raise GitHubServiceError("Failed to generate access token from GitHub")
                
            return token_resp.json()["token"]

    async def create_repo_if_not_exists(self, repo_name: str, access_token: str) -> Dict[str, Any]:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if check_resp.status_code == 200:
                ...
                raise GitHubServiceError(f"Unexpected error checking repo existence: {check_resp.text}")
            ...
            if create_resp.status_code != 201:
                raise GitHubServiceError(f"Failed to create repo {repo_name}: {create_resp.text}")
    ...
    async def enable_pages_via_api(self, repo_name: str, access_token: str) -> None:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if resp.status_code in (201, 204):
                ...
                logger.info("github_pages_already_enabled_or_conflict", repo_name=repo_name)
            else:
                logger.error("github_pages_enable_failed", status=resp.status_code, response=resp.text)
