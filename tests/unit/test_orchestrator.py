...
def test_read_existing_code(mock_exists, orchestrator):
    with patch("builtins.open", patch.mock_open(read_data="file content")):
        content = orchestrator._read_existing_code("/dummy/path", ["file1.py", "file2.py"])
...
@pytest.mark.asyncio
async def test_process_task_happy_path(mock_rag, mock_anthropic, mock_github, orchestrator):
    task = TaskRecord(
    ...
    mock_gh_instance.create_repo_if_not_exists.return_value = "https://github.com/testuser/test"
    mock_gh_instance.enable_pages_via_api.return_value = "https://testuser.github.io/test"
    mock_github.return_value = mock_gh_instance

    mock_llm_instance = AsyncMock()
    mock_llm_instance.generate_code.return_value = {"files": {"test.py": "print('ok')"}, "metadata": {}}
        ...
        with patch.object(orchestrator, "_run_safety_gate", return_value=None):
            with patch("app.services.orchestrator.shutil.rmtree"):
                with patch("app.services.orchestrator.os.makedirs"):
                    with patch("app.services.orchestrator.os.path.join", return_value="/tmp/test.py"):
                        with patch("builtins.open", patch.mock_open()):
                            await orchestrator.process_task(task_id=task.id, instruction="instruction")
